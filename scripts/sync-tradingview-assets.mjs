...
function getSelectedRuntimeKeys() {
  ...
  args.forEach((arg) => {
    ...
    if (!aliases) {
      throw new Error(`Unknown runtime "${arg}". Use "ac", "tp", or "all".`);
    }
...
async function syncRuntime({ label, route, sourcePaths, outputPath }) {
  for (const candidatePath of sourcePaths.filter(Boolean)) {
    if (await exists(candidatePath)) {
      ...
      console.log(`Synced ${label} from ${path.relative(projectRoot, candidatePath)}.`);
      return true;
    }
  }
  ...
  const runtimes = [
    {
      ...
      route: '/',
      sourcePaths: [
        path.join(projectRoot, 'charting_library-master', 'charting_library'),
      ],
    ...
    },
    {
      ...
      route: '/trading',
      sourcePaths: [
        path.join(projectRoot, 'trading_platform-master', 'charting_library'),
      ],
