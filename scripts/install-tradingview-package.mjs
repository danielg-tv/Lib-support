...
async function findInstalledRuntime(nodeModulesPath) {
  ...
  throw new Error('Installed package did not contain a charting_library/ runtime folder.');
}

async function copyBrokerSampleIfPresent(packagePath) {
  const sourcePath = path.join(packagePath, 'broker-sample', 'dist', 'bundle.js');
...
async function downloadAndCopyRuntime(runtime, ref) {
  ...
  const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'tradingview-package-'));
  ...
  try {
    ...
    const { packagePath, runtimePath } = await findInstalledRuntime(path.join(tempRoot, 'node_modules'));

    await rm(runtime.outputPath, { recursive: true, force: true });
    await mkdir(path.dirname(runtime.outputPath), { recursive: true });
    await cp(runtimePath, runtime.outputPath, { recursive: true, force: true });

    if (runtime.repository === 'trading_platform') {
      const copiedBrokerSample = await copyBrokerSampleIfPresent(packagePath);
      if (copiedBrokerSample) {
        console.log('Copied broker sample bundle into third_party/tradingview/broker-sample/dist/bundle.js.');
      } else {
        console.warn('Broker sample bundle was not found in the installed package.');
      }
    }

    console.log(`Copied ${runtime.label} runtime into ${path.relative(projectRoot, runtime.outputPath)}.`);
  } finally {
