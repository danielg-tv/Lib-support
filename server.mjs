...
function decodeHtmlEntities(text) {
  return text
    ...
    .replace(/&#x([0-9a-f]+);/gi, (_, value) => fromHtmlCodePoint(value, 16))
    .replace(/&nbsp;/g, " ")
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
...
function sanitizeRssPresentation(xml) {
  return xml.replace(/<(title|description)>([\s\S]*?)<\/\1>/g, (match, tagName, rawValue) => {
    const cdataMatch = rawValue.match(/^<!\[CDATA\[([\s\S]*)\]\]>$/);
    const value = cdataMatch ? cdataMatch[1] : rawValue;
    const cleaned = cleanRssText(value);

    return cleaned ? `<${tagName}>${toCdata(cleaned)}</${tagName}>` : match;
  });
}
