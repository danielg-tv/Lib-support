...
export class LocalStorageSaveLoadAdapter {
  ...
  constructor({ flushIntervalMs = 1000 } = {}) {
    ...
    this._studyTemplates = this._getFromLocalStorage(STORAGE_KEYS.studyTemplates, []);
    this._drawingTemplates = this._getFromLocalStorage(STORAGE_KEYS.drawingTemplates, []);
    this._chartTemplates = this._getFromLocalStorage(STORAGE_KEYS.chartTemplates, []);
  ...
  removeDrawingTemplate(toolName, templateName) {
    ...
    this._drawingTemplates = this._drawingTemplates.filter(
      (template) => template.name !== templateName || template.toolName !== toolName,
    );
  ...
  getDrawingTemplates() {
    return Promise.resolve(this._drawingTemplates.map((template) => template.name));
  }
  ...
  _saveAllToLocalStorage() {
    ...
    this._saveToLocalStorage(STORAGE_KEYS.studyTemplates, this._studyTemplates);
    this._saveToLocalStorage(STORAGE_KEYS.drawingTemplates, this._drawingTemplates);
    this._saveToLocalStorage(STORAGE_KEYS.chartTemplates, this._chartTemplates);
