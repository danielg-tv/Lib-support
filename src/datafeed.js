...
export default {
	...
	resolveSymbol: async (
		...
		const symbolInfo = {
			...
			fractional: false // Set to true if the symbol has fractional prices
		};
	...
	subscribeDepth(symbol, callback) {
		...
		if (!this._depthSubscriptions) this._depthSubscriptions = {};
		// Clear any previous interval for this symbol
		if (this._depthSubscriptions[symbol]) {
			clearInterval(this._depthSubscriptions[symbol]);
		}

		// Example: Use last price from cache if available, else fallback
		let lastPrice = 3800;
		if (lastBarsCache.has(symbol)) {
			lastPrice = lastBarsCache.get(symbol).close;
		}

		this._depthSubscriptions[symbol] = setInterval(() => {
			// Simulate DOM data around lastPrice
			const bidStart = lastPrice - 2;
			const bidEnd = lastPrice + 2;
			const askStart = lastPrice + 0.01;
			const askEnd = lastPrice + 4;

			const bids = generateDOMData(bidStart, bidEnd, 0.01, 10000);
			const asks = generateDOMData(askStart, askEnd, 0.01, 10000);
	...
	// Optionally, add an unsubscribeDepth method for cleanup:
	unsubscribeDepth(symbol) {
		if (this._depthSubscriptions && this._depthSubscriptions[symbol]) {
			clearInterval(this._depthSubscriptions[symbol]);
			delete this._depthSubscriptions[symbol];
		}
	}
};