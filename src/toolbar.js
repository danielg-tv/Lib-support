...
function enableRovingTabindex(...elements) {
  ...
  elements.filter(Boolean).forEach((element) => {
    element.addEventListener("roving-tabindex:main-element", handleMainElement);
    element.addEventListener("roving-tabindex:secondary-element", handleSecondaryElement);
  });
}
...
export function addThemeToggle(widget) {
  ...
  checkboxEl.checked = typeof widget.getTheme === "function"
...
export function addDocumentationButton(widget) {
  ...
  element.addEventListener("click", () => {
    window.open("https://www.tradingview.com/charting-library-docs/", "_blank");
  });
