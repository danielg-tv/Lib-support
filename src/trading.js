...
import { addDocumentationButton, addThemeToggle, wireRovingTabindex } from "./toolbar.js";
import { createTradingPlatformOptions } from "./widget-options.js";

const BROKER_SAMPLE_SCRIPT_URL = "../third_party/tradingview/broker-sample/dist/bundle.js";
...
// Loads TradingView's broker sample runtime before broker options are created.
function loadBrokerSampleScript() {
  return new Promise((resolve, reject) => {
    const script = document.createElement("script");
    script.src = BROKER_SAMPLE_SCRIPT_URL;
  ...
  return {
    contextMenu: {
      items_processor: async (items, actionsFactory, params) => {
        ...
        if (lastPlusClickPrice === null || lastPlusClickPrice === undefined) return items;
...
function showTradingPlatformWarning() {
  ...
  warning.textContent = "Trading Platform broker features did not initialize. Make sure trading_platform-master was synced and the broker sample bundle exists.";
...
function addVisibilityButton(widget) {
  ...
  function updateLabel() {
    button.textContent = visualsHidden ? "Show Indicators/Drawings" : "Hide Indicators/Drawings";
  }
  ...
  button.addEventListener("click", () => {
    ...
    widget.activeChart().getAllStudies().forEach((study) => {
      widget.activeChart().getStudyById(study.id).setVisible(!visualsHidden);
    });
    updateLabel();
  });
}
...
function installChartReadySubscriptions(widget, alertController) {
  widget.subscribe("onAutoSaveNeeded", () => {
    if (typeof widget.saveChartToServer === "function") {
      widget.saveChartToServer(null, null, { defaultChartName: "Default" });
    }
  });
...
async function initTradingPlatformChart() {
  try {
    await loadBrokerSampleScript();
  } catch {
    // The visible warning below explains the missing Trading Platform broker runtime.
  }
  ...
  widget = new TradingView.widget(createTradingPlatformOptions({
    widgetbar: {
      details: true,
      watchlist: true,
      datawindow: true,
  ...
  widget.onChartReady(() => {
    ...
    installChartReadySubscriptions(widget, alertController);
  });

  installToolbar(widget);
}

window.addEventListener("DOMContentLoaded", initTradingPlatformChart, { once: true });
